...
<div class="single-post">
    ...
    <div class="icons">
        <div class="left-area">
            <a class="" href="#"><b>{{ $infos->judul_info }}</b></a>
        </div>
        <ul class="right-area social-icons">
            <li><a href="#"><i class="ion-android-textsms"></i>{{ count($data['comment']) }}</a></li>
        </ul>
    </div>
    <p class="date"><em>{{ date('D, M Y', strtotime($infos->created_at)) }}</em></p>
    <h3 class="title"><a href="#">{{ $infos->isi_info }}</b></a></h3>
    {!! $infos->content !!}
...
<div class="leave-comment-area">
    ...
    <div class="leave-comment">
        ...
        <form method="post" action="{{ url('comment') }}">
            @csrf
            <input type="hidden" name="isi_info" value="{{ $infos->id }}">
            <div class="row">
                <div class="col-sm-6">
